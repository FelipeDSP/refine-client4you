-- Update plans for Client4you rebranding - FIXED VERSION
-- New plans: demo, basico, intermediario, avancado

-- Step 1: Update existing data first (before constraint change)
UPDATE user_quotas
SET 
...
-- Step 2: Update constraint to accept new plan types
...
-- Step 3: Drop old function explicitly with signature
DROP FUNCTION IF EXISTS check_user_quota(UUID, VARCHAR);
DROP FUNCTION IF EXISTS upgrade_user_plan(UUID, VARCHAR, VARCHAR);
DROP FUNCTION IF EXISTS reset_monthly_quotas();

-- Step 4: Create upgrade_user_plan function with new plans
CREATE FUNCTION upgrade_user_plan(
  p_user_id UUID,
  p_plan_type VARCHAR,
  p_plan_name VARCHAR
)
...
-- Step 5: Create check_user_quota function with new plan names
CREATE FUNCTION check_user_quota(
  p_user_id UUID,
  p_action VARCHAR
)
RETURNS JSONB AS $$
...
-- Step 6: Create reset_monthly_quotas
CREATE FUNCTION reset_monthly_quotas()
RETURNS void AS $$
BEGIN
  UPDATE user_quotas
  SET 
