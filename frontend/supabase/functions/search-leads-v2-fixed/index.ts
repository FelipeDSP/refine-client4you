...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Verificar autenticação (mais flexível)
    ...
    console.log(`[search-leads-v2] Action: ${action}, User: ${user?.id || 'anonymous'}`);

    // Se não tem usuário autenticado, retornar erro amigável
    if (!user || !userProfile) {
      return new Response(
        JSON.stringify({ 
    ...
    // Usar company_id do profile em vez do body
    const effectiveCompanyId = userProfile.company_id || company_id;

    if (!effectiveCompanyId) {
      return new Response(
        JSON.stringify({ error: "Company ID not found" }),
    ...
    if (action === 'create') {
      const { query, location, search_type = 'serp' } = body;
      ...
      const results = await fetchSearchResults(supabaseAdmin, session, 0);
    ...
    // ===== ACTION: FETCH MORE RESULTS =====
      ...
      const { data: session, error: sessionError } = await supabaseAdmin
        ...
        .select('*')
        .eq('id', session_id)
        .eq('company_id', effectiveCompanyId) // Validar ownership
        .single();
      ...
      const results = await fetchSearchResults(supabaseAdmin, session, nextPage);
...
async function fetchSearchResults(supabase: any, session: any, page: number) {
  ...
  const serpapiKey = company?.settings?.serp_api_key;
